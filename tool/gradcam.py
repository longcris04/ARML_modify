...
class GradCam:
    ...
    '''
    def __init__(self, model, feature_module, target_layer_names, use_cuda, device=None):
        self.model = model
        self.feature_module = feature_module
        self.model.eval()
        self.cuda = use_cuda
        # Get device from model parameters or use provided device
...
class SmoothScoreCAM:
    ...
    def generate(self, input_image, target_class, sigma=0.2):
        input_image = input_image.cuda()
        self.model.eval()

        self.model.zero_grad()
        if target_class is None:
            target_class = self.model(input_image).argmax()
        ...
        # Calculate per-channel importance
        gradients = self.activations.grad[0]
        weights = torch.mean(gradients, dim=(1, 2))

        # Generate class activation map
        cam = torch.sum(weights.view(-1, 1, 1) * self.activations, dim=0)
        cam = F.relu(cam)
        cam = F.interpolate(cam.unsqueeze(0), size=(input_image.size(2), input_image.size(3)), mode='bilinear', align_corners=False)
        cam = cam - cam.min()
        cam = cam / (cam.max() + 1e-8)

        # Smooth the CAM with a Gaussian filter
        cam = cam.cpu().numpy()
        cam = cv2.GaussianBlur(cam, (21, 21), sigma)
